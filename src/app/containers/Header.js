...
const Header = ({ headerIconVisible }) => (
  <Box.fixed is="header" w={1} zIndex={99}>
    <Flex justify="space-between" align="center" height={theme.headerHeight} bg={headerIconVisible ? 'fade.white.70' : null} is={Box.transition}>
      <Box w={searchWidth} display={['none', 'none', 'block']} />
      <Box px="1em">
        <Logo.horizontal w="12em" opacity={Number(headerIconVisible)} is={Box.transition} />
      </Box>
